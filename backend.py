...
def save_to_db(lat, lon, count):
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?)
    """, (lat, lon, count, str(datetime.now())))
    conn.commit()
...
def process_video(file_path, lat, lon):
    ...
    while True:
        ...
        if count > 0 and current_time - last_alert_time > cooldown:
            ...
            msg = f"""🚧 Pothole Detected in Video
Count: {count}
Time: {datetime.now()}
GPS: {lat}, {lon}"""
...
async def detect(
    ...
    if file.filename.lower().endswith((".jpg", ".jpeg", ".png")):
        return process_image(file_path, lat, lon)

    elif file.filename.lower().endswith((".mp4", ".avi", ".mov")):
        return process_video(file_path, lat, lon)
